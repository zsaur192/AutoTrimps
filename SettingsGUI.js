...
//Spire
    ...
    createSetting('PraidHarder', 'P Raiding 2', '(EXPERIMENTAL) PRaid Harder: When enabled, always buys the highest prestige map we can afford when P raiding, with option to farm fragments for highest available prestige level.','boolean',false,null,'Spire');
    createSetting('PraidFarmFrags', 'Farm Fragments', 'Farm fragments until we can afford the highest prestige map for P raiding','boolean',true,null,'Spire');
    createSetting('PraidBeforeFarm', 'Raid before farming', 'P Raid as far as we can afford before trying to farm fragments to Praid the highest prestige map.  Only occasionally useful, e.g. if it picks up a Speedexplorer or farming fragments is slow due to low damage.  Will probably be switched to a multivalue setting at some point after testing.','boolean', false, 'Spire');
