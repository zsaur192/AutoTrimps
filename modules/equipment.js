...
function autoLevelEquipment() {
    ...
    for (var stat in Best) {
        ...
        if (eqName !== '') {
            ...
            var ultwind = (getPageSetting('ultwind') >= 1 && game.global.world >= getPageSetting('ultwind') && HDratioy() < getPageSetting('ultwindcut') && game.global.challengeActive != "Daily");
            var dultwind = (getPageSetting('dultwind') >= 1 && game.global.world >= getPageSetting('dultwind') && HDratioy() < getPageSetting('dultwindcut') && game.global.challengeActive == "Daily");
            var hardcorewind = (getEmpowerment() == "Wind" && HDratioy() <= getPageSetting('windcutoff') && getPageSetting('hardcorewind') >= 1 && game.global.world >= getPageSetting('hardcorewind') && (game.global.world < getPageSetting('hardcorewindmax') || getPageSetting('hardcorewindmax') <= 0) && game.global.challengeActive != "Daily");
            var dhardcorewind = (getEmpowerment() == "Wind" && HDratioy() <= getPageSetting('dwindcutoff') && getPageSetting('dhardcorewind') >= 1 && game.global.world >= getPageSetting('dhardcorewind') && (game.global.world < getPageSetting('dhardcorewindmax') || getPageSetting('dhardcorewindmax') <= 0) && game.global.challengeActive == "Daily");
            var notWindstack = !ultwind && !dultwind && !hardcorewind && !dhardcorewind;
            if (BuyWeaponLevels && DaThing.Stat == 'attack' && notWindstack && (!enoughDamageE || enoughHealthE || maxmap || spirecheck)) {
