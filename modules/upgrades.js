...
function buyUpgrades() {
    ...
    for (var upgrade in upgradeList) {
		...
		if (upgrade == 'Coordination' && getPageSetting('amalcoord')==true && getPageSetting('amalcoordhd') > 0 && HDratioy() < getPageSetting('amalcoordhd') && ((getPageSetting('amalcoordt') < 0 && (game.global.world < getPageSetting('amalcoordz') || getPageSetting('amalcoordz') < 0)) || (getPageSetting('amalcoordt') > 0 && getPageSetting('amalcoordt') > game.jobs.Amalgamator.owned && (game.resources.trimps.realMax() / game.resources.trimps.getCurrentSend()) > 2000))) continue;
	
		//WS
		if (upgrade == 'Coordination' && !canAffordCoordinationTrimps() && getEmpowerment() == "Wind" && ((game.global.challengeActive != "Daily" && getPageSetting('WindStackingMin') > 0 && getPageSetting('WindStackingMinHD') > 0 && game.global.world >= getPageSetting('WindStackingMin') && HDratioy() < getPageSetting('WindstackingMinHD')) || (game.global.challengeActive == "Daily" && getPageSetting('dWindStackingMin') > 0 && getPageSetting('dWindStackingMinHD') > 0 && game.global.world >= getPageSetting('dWindStackingMin') && HDratioy() < getPageSetting('WindstackingMinHD')))) continue;
		if (upgrade == 'Coordination' && !canAffordCoordinationTrimps() && ((game.global.challengeActive != "Daily" && getPageSetting('wsmax') > 0 && getPageSetting('wsmaxhd') > 0 && game.global.world >= getPageSetting('wsmax') && HDratioy() < getPageSetting('wsmaxhd')) || (game.global.challengeActive == "Daily" && getPageSetting('dwsmax') > 0 && getPageSetting('dwsmaxhd') > 0 && game.global.world >= getPageSetting('dwsmax') && HDratioy() < getPageSetting('wsmaxhd')))) continue;
