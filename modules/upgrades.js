...
function buyUpgrades() {
    for (var upgrade in upgradeList) {
        ...
        if (upgrade == 'Coordination' && (getPageSetting('BuyUpgradesNew') == 2 || !canAffordCoordinationTrimps() || (getPageSetting('amalcoord')==true && (game.global.world < getPageSetting('amalcoordz') || getPageSetting('amalcoordz') < 0) && (getPageSetting('amalcoordt') < 0 && HDratioy() < getPageSetting('amalcoordhd')) || (getPageSetting('amalcoordt') > 0 && getPageSetting('amalcoordt') == game.jobs.Amalgamator.owned && (game.resources.trimps.realMax() / game.resources.trimps.getCurrentSend()) < 2000)))) continue;
