...
function buyUpgrades() {
	...
	//WS
	if (upgrade == 'Coordination' && !canAffordCoordinationTrimps() && getEmpowerment() == "Wind" && ((game.global.challengeActive != "Daily" && getPageSetting('WindStackingMin') > 0 && getPageSetting('WindStackingMinHD') > 0 && game.global.world >= getPageSetting('WindStackingMin') && calcHDratio() < getPageSetting('WindstackingMinHD')) || (game.global.challengeActive == "Daily" && getPageSetting('dWindStackingMin') > 0 && getPageSetting('dWindStackingMinHD') > 0 && game.global.world >= getPageSetting('dWindStackingMin') && calcHDratio() < getPageSetting('WindstackingMinHD')))) continue;
	if (upgrade == 'Coordination' && !canAffordCoordinationTrimps() && ((game.global.challengeActive != "Daily" && getPageSetting('wsmax') > 0 && getPageSetting('wsmaxhd') > 0 && game.global.world >= getPageSetting('wsmax') && calcHDratio() < getPageSetting('wsmaxhd')) || (game.global.challengeActive == "Daily" && getPageSetting('dwsmax') > 0 && getPageSetting('dwsmaxhd') > 0 && game.global.world >= getPageSetting('dwsmax') && calcHDratio() < getPageSetting('wsmaxhd')))) continue;
