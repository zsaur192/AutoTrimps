...
if (getPageSetting('UseScryerStance') == true  && ((valid_min && valid_max && !(game.global.mapsActive && getPageSetting('onlyminmaxworld') == true)) || game.global.mapsActive && [game.global.mapGridArray[(getCurrentMapCell().level)%game.global.mapGridArray.length]["name"], getCurrentMapCell().name].some(o => ["Jestimp", "Chronoimp"].includes(o)))) {
    if (oktoswitch)
    setFormation(4);
    wantToScry = true;
    }
